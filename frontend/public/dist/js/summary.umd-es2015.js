...
var base64$1 = (base64 && typeof base64 === 'object' && 'default' in base64 ? base64['default'] : base64);
...
function chart(id) {
  ...
  function _impl(context) {
    ...
    selection.each(function() {
      ...
      var _imageId = function (d) { return ("image-" + (base64$1.encode(d.data.u || d.data.l || d.data.v).replace(/\+/g, '-').replace(/\//g, '_').replace(/\=+$/, ''))); }
      if(appendImage){
...
var SummaryView = (function (SiftView) {
  ...
  SummaryView.prototype.onStorageUpdate = function onStorageUpdate (data) {
    if(!this._selected) {
  ...
  SummaryView.prototype._getMonth = function _getMonth (month, data) {
    if (month && data) {
  ...
  SummaryView.prototype._updateSelection = function _updateSelection (ev) {
    this._selected = ev.target.value;
    this._updateScreen(this._selected);
  };

  SummaryView.prototype._updateScreen = function _updateScreen (selected) {
  ...
  SummaryView.prototype._updateMonths = function _updateMonths () {
    var months = this._data || [];
    if (months.length !== this._months) {
      ...
      for (var i = 0; i < months.length; i++) {
        ...
        option.textContent = moment$1(months[i].month, 'YYYYMM').format('MMM YY');
  ...
  SummaryView.prototype._updateStats = function _updateStats (selected) {
    var month = this._getMonth(selected, this._data);
    if (month) {
      var data = month;
      var percent = 0;
  ...
  SummaryView.prototype._updateTreeMap = function _updateTreeMap (selected, delay) {
    var month = this._getMonth(selected, this._data);
    var data;
    if (month) {
      data = month.treemap;
    }
    var w = select('#treemap').node().offsetWidth;
    var h = select('#home').node().offsetHeight;
    if (w && h) {
      if (!this._tm) {
        this._tm = chart('pixel-tracker')
          .appendImage(true)
          .fill(['#ccc'])
          .imageFallbackLink('assets/eye-purple.svg');
        select('#treemap').datum(data).call(this._tm.width(w).height(h));
        this._tip = tip().html(function (d) { return 'Company: ' + d.data.l + '<br>Tracked emails: ' + d.data.v; });
      }
      else {
  ...
  SummaryView.prototype._updateHero = function _updateHero () {
    var current = this._getMonth(moment$1().format('YYYYMM'), this._data);
    if (current) {
      var percent = 0;
      if (current.emails && current.tracked) {
        percent = (100 * current.tracked / current.emails).toFixed(0);
      }
      var pt = document.getElementsByClassName('currentpercent');
