...
function chart(id) {
  ...
  function _impl(context) {
    ...
    selection.each(function() {
      ...
      let _imageId = (d) => `image-${ base64$1.encode(d.data.u || d.data.l || d.data.v).replace(/\+/g, '-').replace(/\//g, '_').replace(/\=+$/, '') }`
      if(appendImage){
...
class SummaryView extends SiftView {
  ...
  _updateHero() {
    let current = this._getMonth(moment$1().format('YYYYMM'), this._data);
    if (current) {
      let percent = 0;
